...
```
# 创建前置任务
task_create(subject="安装依赖", description="pip install -r requirements.txt")
# 返回 id=1

# 创建依赖任务（必须先完成 id=1）
task_create(subject="运行测试", blocked_by=[1])
# 返回 id=2
...
场景 - 使用工具 - 原因：
- 写个简单脚本 - TodoManager - 线性流程，无依赖
- 重构一个模块 - TodoManager - 顺序执行即可
- 搭建开发环境 - TaskManager - 多个任务有依赖关系
- 执行CI/CD流程 - TaskManager - 有条件分支和依赖
- 复杂项目迁移 - TaskManager - 大量任务需要编排

## 任务状态更新流程

执行操作后检查结果：
- 成功：标记 completed，继续下一步
